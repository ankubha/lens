...
export default function ProfilePage() {
  ...
  if (error) return (
    <div style={{
      ...
      border: '0.5px solid rgba(204,34,34,0.3)',
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              <Badge
                variant={
                  ...
                  isUnstructured ? 'unstructured' : 'semi'
                }
                label={
                  isStructured ? 'Structured' :
                  isUnstructured ? 'Unstructured' : 'Semi-structured'
                }
                dot
              />
              ...
              {profile.sensitivity_classification && (
                <Badge
                  variant={profile.sensitivity_classification as any}
                  label={profile.sensitivity_classification}
                />
              )}
            </div>
            ...
            <div style={{
              ...
              {[
                ...
                {
                  label: 'File Size',
                  value: `${(profile.file_size_bytes / 1024).toFixed(1)} KB`,
                },
                isStructured && profile.row_count != null && {
                  label: 'Rows',
                  value: profile.row_count.toLocaleString(),
                },
                isStructured && profile.column_count != null && {
                  label: 'Columns',
                  value: profile.column_count.toString(),
                },
                isUnstructured && profile.page_count != null && {
                  label: 'Pages',
                  value: profile.page_count.toString(),
                },
                isUnstructured && {
                  label: 'Language',
                  value: profile.detected_language?.toUpperCase() || 'EN',
                },
                profile.llm_used && {
                  label: 'Model',
                  value: profile.llm_used,
                },
        ...
        <div style={{
          ...
          {isStructured && [
            ...
            },
            {
              ...
              value: profile.duplicate_row_count?.toString() || '0',
              sub: 'exact duplicates',
              alert: (profile.duplicate_row_count || 0) > 0,
            },
            {
              label: 'PII Columns',
              value: profile.columns.filter(c => c.is_pii).length.toString(),
              sub: 'flagged',
              alert: profile.columns.filter(c => c.is_pii).length > 0,
            },
            {
              label: 'Missing Data',
              value: profile.columns.filter(c => c.missing_pct > 0).length.toString(),
              sub: 'columns affected',
              alert: profile.columns.some(c => c.missing_pct > 10),
            },
          ].map((stat: any) => (
            <div key={stat.label} style={{
              ...
              <div style={{
                fontSize: 24,
                fontWeight: 600,
                color: stat.alert ? 'var(--wf-red)' : 'var(--ink)',
                lineHeight: 1.2,
                fontFamily: 'var(--font-display)',
              }}>
                {stat.value}
          ...
          {isUnstructured && [
            ...
            },
            {
              ...
              value: `${profile.found_fields_count || 0}/${profile.expected_fields_count || 0}`,
              sub: 'critical elements',
            },
            {
              label: 'Completeness',
              value: `${profile.completeness_score?.toFixed(0) || 0}%`,
              sub: 'schema coverage',
              alert: (profile.completeness_score || 0) < 70,
            },
            {
              label: 'Document Type',
              value: profile.document_type_confidence
                ? `${(profile.document_type_confidence * 100).toFixed(0)}%`
                : '—',
              sub: 'confidence',
            },
            {
              label: 'Additional Findings',
              value: profile.additional_findings?.length.toString() || '0',
              sub: 'extra facts found',
            },
          ].map((stat: any) => (
            <div key={stat.label} style={{
              background: stat.alert ? 'var(--wf-red-light)' : 'var(--surface-2)',
